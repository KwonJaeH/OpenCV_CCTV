...
# OpenCV_CCTV
실시간 CCTV 영상 기반, 범위 내 차량 속도 추정  :oncoming_automobile:   
...
> 그래서 이동체 추적으로 속도 추정 범위 내에 차량이 있는지 계속 추적해야지 범위를 벗어날 때 값을 도출할 수 있었다. 
...
## 개선해야할 점  
>
> - 프레임에 이동체의 중심 좌표가 찍힌 기준으로 계산하기 때문에 정확히 70m의 이동거리를 가지지 못한다.(추정일뿐)     
